...
# Alternative: cellranger count matrix
# data_path<-"data/BCC_processed_matrices/cellranger_matrices/BCC06/"
# count_matrix<-readMM(paste0(data_path,
...
#              header=FALSE)
# colnames(count_matrix)<-cells$V1
# rownames(count_matrix)<-genenames$V2
# 
# count_matrix<-apply(count_matrix,2,tapply,as.factor(genenames$V2),sum,na.rm=T)
...
# ------------------------------------------------------------------------------
# BCC06 - pre treatment (called BCC06 in the workflow)
# ------------------------------------------------------------------------------
...
meta_data$cluster[startsWith(meta_data$cluster,"Tumor")]<-"BCC06"

#Save results
write.table(count_matrix,
            file="data/BCC_processed_matrices/input_BCC06/count_matrix.txt",
            sep="\t",quote=FALSE,row.names=TRUE)

write.table(meta_data[,c("cell.id","cluster")], 
            file="data/BCC_processed_matrices/input_BCC06/sample_annotation.txt",
            sep="\t",quote=FALSE,row.names=FALSE,col.names=FALSE)

df_refs<-data.frame(ref_groups=setdiff(unique(meta_data$cluster),"BCC06"))
write.table(df_refs, 
            file="data/BCC_processed_matrices/input_BCC06/ref_groups.txt",
...
# ------------------------------------------------------------------------------
# BCC06 - post treatment (called BCC06post in the workflow)
# ------------------------------------------------------------------------------
