...
Important note: exclude SNU16 for now, because we have no current Numbat results for it!
...
# ------------------------------------------------------------------------------
# Deviation dependent on reference dataset 
# ------------------------------------------------------------------------------
...
pred_comb<-NULL
for(annot_data in c("MM","BCC06","BCC06post")){
...
rename_metrics<-setNames(
         c("Pearson corr","Trunc AUC (gains)","Trunc AUC (losses)","Max F1 Score",
           "RMSE (matched ref)","RMSE (other cell type)","RMSE (other dataset)",
           "RMSE (other cell type & dataset)",
           "Accuracy","ARI","Homogeneity",
           "Runtime (h)","Memory (GB)"),
         c("pearson","auc_gains_trunc","auc_losses_trunc","max_f1",
...
# g<-ggplot(all_evals,aes(y=method,x=metric,fill=norm_value))+
#   geom_tile()+xlab("Metric")+
...
g<-ggplot(all_evals,aes(y=method,x=metric,color=norm_value,size=norm_value))+
...
ggsave(g,file="../figure_plots/summary_plot.png")
```
...
# ------------------------------------------------------------------------------
# Plot about runtime and memory consumption
# ------------------------------------------------------------------------------
...
g.4<-ggplot(resources,aes(x=mean_runtime_h,y=mean_max_vms_gb,color=method))+
...
g.bottom<-ggarrange(g.3,g.4,labels=LETTERS[3:4],widths=c(0.6,0.4),font.label=list(size=21))
g<-ggarrange(g.1,g.2,g.bottom,labels=LETTERS[1:2],heights=c(0.5,0.25,0.25),
...
# #AUC truncated (gain)
# g.2<-ggplot(res_cell,aes(x=auc_trunc_gain,fill=method))+
#   geom_histogram()+
#   xlab("Truncated AUC (gain)")+
#   geom_vline(data=res_complete[res_complete$metric=="auc_gains_trunc"],aes(xintercept=value))+
#   facet_wrap(~method)+
#   xlim(0,1)+
#   theme(legend.position="none")
# 
# #AUC truncated (loss)
# g.3<-ggplot(res_cell,aes(x=auc_trunc_loss,fill=method))+
#   geom_histogram()+
#   xlab("Truncated AUC (loss)")+
#   geom_vline(data=res_complete[res_complete$metric=="auc_losses_trunc"],aes(xintercept=value))+
#   facet_wrap(~method)+
#   xlim(0,1)+
#   theme(legend.position="none")

#g<-ggarrange(g.1,g.2,g.3,ncol=1)
print(g.1)

ggsave(g.1,file="../figure_plots/suppfig_per_cell_performance.png")
...
## Figure 3 - Performance dependent on the reference dataset
...
# orginal_vals<-tmp$MCF7_epithelial
# for(dataset in dataset_names){
...
# 
# #Rename facet headers
# panelnames<-setNames(c("Correlation","AUC (Gain)","AUC (Loss)"),
...
g<-ggarrange(g.1,g.2,labels=LETTERS[1:2],ncol=1,
             common.legend=TRUE,legend="bottom")
print(g)

ggsave(g,file="../figure_plots/fig3_performance_refs.png")
```

Figure: Performance for predicting CNVs in the MCF7 dataset (A) and the MM dataset (B) when using different reference datasets.

## Figure 4 - Identification of diploid datasets
...
g.3<-ggplot(mse_perf,aes(x=method,y=rmse,fill=method))+
...
g<-ggarrange(g.1,g.2,g.3,heights=c(0.32,0.35,0.4),
             labels=LETTERS[1:3],font.label=list(size=21),ncol=1)
print(g)

ggsave(g,file="../figure_plots/fig4_performance_diploid.png")

```


## Figure 5 - Subclone identification
...
g.1<-cowplot::plot_grid(g.1.1,g.1.2,g.1.3,ncol=3,
                        rel_widths=c(0.1,1,0.2),axis="b",align="h")
...
g.2<-ggarrange(g.2.1,g.2.2,labels=LETTERS[2:3],font.label=list(size=21))
...
rename_metrics<-setNames(
         c("Maximal F1 Score","Correlation","Truncated AUC (gain)","Truncated AUC (loss)"),
...
g<-ggplot(mse,aes(x=rmse_cr1,y=rmse_cr7,color=Method))+
  geom_point()+geom_abline()+
  xlab("RMSE (reference mapped with CellRanger V1)")+
  ylab("RMSE (reference mapped with CellRanger V7)")
