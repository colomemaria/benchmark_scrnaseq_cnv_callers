...
g<-ggarrange(g.1,NULL,g.bottom,NULL,g.2,labels=c(LETTERS[1],"","","",LETTERS[4]),heights=c(0.42,0.02,0.30,0.02,0.24),
...
g.bottom<-ggarrange(g.2,g.3,ncol=2,labels=LETTERS[2:3],font.label=list(size=21),
...
g<-ggarrange(g.1,g.bottom,heights=c(0.5,0.5),
             labels=c(LETTERS[1],""),font.label=list(size=21),ncol=1)
print(g)
...
g.4 <-ggplot(plot_data,aes(x=counted_pos,y=variable,fill=value))+geom_tile()+
  ...
  scale_fill_gradient2("Score",low = "darkblue",
                       ...
                       labels=c("loss","base","gain"))+
  xlab("Chromosome position")+ylab("Method")+
  ggtitle("CD14+ Monocytes (ext ref)")+
  geom_vline(xintercept = chr_boundries$start_chr)+
  scale_x_continuous(breaks=chr_boundries$mean_chr,
                     labels=chr_boundries$chr)+
  scale_y_discrete(limits=rev)+
...
g<-ggarrange(g.1,g.2,g.3,g.4,heights=c(0.24,0.24,0.24,0.28),
             labels=LETTERS[1:4],font.label=list(size=21),ncol=1)
print(g)
